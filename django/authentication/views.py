...
class OAuthCallbackView(APIView):
    ...
    def get(self, request):
        ...
        if not code:
            return HttpResponseRedirect("https://" + os.environ.get("ALLOWED_HOST") + ":443")
        ...
        redirect_uri =  "https://" + HOST + "/auth/oauth/callback/"
...
class GoogleOAuthCallbackView(APIView):
    ...
    def get(self, request):
        ...
        if "error" in token_response:
            logger.error(f'Error in token response: {token_response}')
            return redirect('/login')
        ...
        id_info = id_token.verify_oauth2_token(
            token_response['id_token'], google_requests.Request(), os.environ.get("GOOGLE_CLIENT_ID")
        )
        ...
        if user and user.is_authenticated:
            logger.info(f'User {user.username} is logged in')
            login(request, user)
        else:
