...
export function renderLoginPage() {
    ...
    document.getElementById('btn-oauth').addEventListener('click', () => {
        window.location.href = "https://api.intra.42.fr/oauth/authorize?client_id=u-s4t2ud-44e84abb8e03d12d49b46f696a8f77ac97f795feaf74f2be6f54a69b18718c29&redirect_uri=https%3A%2F%2Fft-transcendence.pro%2Fauth%2Foauth%2Fcallback%2F&response_type=code";
    });
    document.getElementById('btn-oauth-google').addEventListener('click', () => {
        window.location.href = `https://accounts.google.com/o/oauth2/v2/auth?client_id=658251774353-pl1dq0h8d4md4e7ejjk4hgpbl9vcd36r.apps.googleusercontent.com&redirect_uri=https://ft-transcendence.pro/auth/oauth/google/callback/&response_type=code&scope=email profile`;
    });
}
...
function getLoginFormHtml() {
    return /*html*/`
        <form action="#" class="login-form active">
            <button type="button" id="btn-oauth" class="button button--login" aria-label="Authenticate using 42 OAuth">Sign in with 42</button>
            <button type="button" id="btn-oauth-google" class="button button--login button--google" aria-label="Authenticate using 42 OAuth">Sign in with <span>G</span><span>o</span><span>o</span><span>g</span><span>l</span><span>e</span></button>
            <p class="login-form__delimitation">or</p>
            <div class="login-form__input-container">
                <div class="form-field">
                    <label for="log_username" class="form-field__label">Username</label>
                    <input type="text" required name="username" id="log_username" class="form-field__input" autocomplete="off" placeholder="Thomas_Toketchup">
                </div>
                <div class="form-field">
